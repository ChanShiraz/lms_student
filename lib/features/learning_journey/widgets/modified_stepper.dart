...
/// The state of a [Step2] which is used to control the style of the circle and
/// text.
///
/// See also:
///
...
/// Defines the [Stepper2]'s main axis.
enum StepperType {
...
///
/// Used by [Stepper2.controlsBuilder].
@immutable
class ControlsDetails {
  ...
  /// Index that is active for the surrounding [Stepper2] widget. This may be
  ...
  /// just activated the transition to a new step, some [Stepper2.type] values will
...
/// A builder that creates the icon widget for the [Step2] at [stepIndex], given
/// [stepState].
...
/// A material step used in [Stepper2]. The step can have a title and subtitle,
...
///
/// {@tool dartpad}
/// An example the shows how to use the [Stepper2], and the [Stepper2] UI
/// appearance.
///
...
class Stepper2 extends StatefulWidget {
  ...
  /// The [Step2.content] will be clipped to this Clip type.
  ///
...
class _Stepper2State extends State<Stepper2> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(Stepper2 oldWidget) {
  ...
  bool _isLabel() {
    for (final Step2 step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget _buildLine(bool visible, bool isActive, double connectorSize) {
    return ColoredBox(
      color: _connectorColor(isActive),
      child: SizedBox(
        width: visible ? widget.connectorThickness ?? 1.0 : 0.0,
        height: 16.0 * connectorSize,
      ),
    );
  }
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    ...
    final Color iconColor = isDarkActive
        ? _kCircleActiveDark
        : _kCircleActiveLight;
    ...
    return switch (state) {
      StepState.indexed ||
      StepState.disabled => Text('${index + 1}', style: textStyle),
  ...
  Widget _buildVerticalHeader(int index) {
    final bool isActive = widget.steps[index].isActive;
    final double size = widget.steps[index].connectorSize;
    return Padding(
        ...
        children: <Widget>[
          Column(
            children: <Widget>[
              ...
              _buildLine(!_isFirst(index), isActive, size),
              _buildIcon(index),
              _buildLine(!_isLast(index), isActive, size),
            ],
          ),
          Expanded(
            child: Padding(
  ...
  Widget _buildVerticalBody(int index) {
    ...
    final double? marginRight = _stepIconMargin
        ?.resolve(TextDirection.ltr)
        .right;
    final double? additionalMarginLeft = marginLeft != null
        ? marginLeft / 2.0
        : null;
    final double? additionalMarginRight = marginRight != null
        ? marginRight / 2.0
        : null;
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<Stepper2>() != null) {
        throw FlutterError(
...
/// This class is used to override the default visual properties of [Step2] widgets within a [Stepper2].
///
/// To customize the appearance of a [Step2] create an instance of this class with non-null parameters
...
///
/// {@tool dartpad}
/// An example that uses [StepStyle] to customize the appearance of each [Step2] in a [Stepper2].
///
...
class StepStyle with Diagnosticable {
  ...
  ///
  /// This property only applies when [Stepper2.type] is [StepperType.horizontal].
